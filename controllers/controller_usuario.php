...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $_POST['id']) {
    ...
    if (isset ($_GET['delete'])) {
        ...
        if (isset ($_GET['bloquearOuDesbloquer'])) {

            $resultado = $dados->bloquearOuDesbloquearUsuario($id_usuario, $_GET['bloquearOuDesbloquer']);
            if ($resultado === TRUE) {
                if ($_GET['bloquearOuDesbloquer'] == 0) 
                    $_SESSION['mensagem_sucesso'] = "Bloqueado feito com sucesso!";
                else
                $_SESSION['mensagem_sucesso'] = "Desbloqueio feito com sucesso!";

            } else
                $_SESSION['mensagem_erro'] = "Falha na operação!";
        }
